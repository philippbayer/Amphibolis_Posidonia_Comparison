...
#annAT <- readMappings('BACKGROUND.txt', sep="\t", IDsep=";")
...
for (f in names(results_list)) {
  ...
  one.data$pvalue <- p.adjust(exp(one.data$value))
  #one.data <- one.data %>% filter(pvalue < 0.05) # NO FILTERING OF P-VALUES - for now? 

  p1 <-
    ggplot(data = one.data, aes(plot_X, plot_Y, label = description)) +
    geom_point(aes(colour = log(pvalue), size = log_size), alpha = I(0.6)) +
    scale_size_area() +
    scale_colour_gradientn(colours = rev(wes_palette("Zissou1", 100, type = "continuous")), 
                           limits = c(min(log(one.data$pvalue)), 0)) +
    geom_point(
    ...
    scale_size_area() +
    geom_label_repel(max.overlaps = 5, box.padding = 0.5,
